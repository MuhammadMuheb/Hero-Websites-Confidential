 ...
 * Colosseum/Private Vatican/Pompeii Day Trip/Rome Vespa's SIBLING_SITES/
 * NETWORK_NAV_ITEMS pattern exactly.
 */
const SIBLING_SITES = NETWORK_SITES.filter((site) => site.slug !== 'golf-cart-rome');
...
/**
 * Shared chrome for every Golf Cart Rome page — home and all 12 money /
 * support / utility pages built off the property's blueprint (Master Network
 * Blueprint §3.7). Mirrors RVShared.tsx / PDTShared.tsx's structure, themed
 * with Sunny Amber (`--accent`, scoped via the `.gcr-scope` wrapper each
 * top-level page component applies — see globals.css) and Public Sans
 * extrabold headings with generous letter-spacing for a rounder, friendlier
 * feel (blueprint §3.7.2) — the accessibility/comfort/family framing that
 * deliberately contrasts with Rome Vespa's two-wheel/adrenaline positioning.
 */

export function GCRBrandMark() {
  return (
    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-accent-gradient shadow-card-soft">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <circle cx="7.5" cy="18" r="2" stroke="white" strokeWidth="1.6" />
        <circle cx="16.5" cy="18" r="2" stroke="white" strokeWidth="1.6" />
        <path
          d="M4 18h1.7M9.3 18h5.4M15.8 18H19c.9 0 1.4-1 .9-1.7l-2-2.8a2 2 0 0 0-1.6-.8H14l-1.2-4a1.6 1.6 0 0 0-1.5-1.1H8.4c-.7 0-1.3.5-1.4 1.2L6 13"
          stroke="white"
          strokeWidth="1.6"
          strokeLinecap="round"
 ...
 */
export function GCRHeader({ toursHref = '#tours', planHref = '#ride-prep', faqHref = '#faq', ctaHref = '#featured-tours' }: {
  toursHref?: string;
  planHref?: string;
  faqHref?: string;
  ctaHref?: string;
    ...
    >
      <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between gap-4 px-6 sm:px-14">
        ...
        <nav aria-label="Primary" className="hidden min-w-0 items-center gap-8 lg:flex">
          ...
          <NavDropdown label="Plan Your Ride" items={PLAN_NAV_ITEMS} footerHref={planHref} footerLabel="See all guides" />
      ...
      {mobileOpen ? (
        <div className="max-h-[calc(100vh-72px)] overflow-y-auto border-t border-line bg-white px-6 py-5 sm:px-14 lg:hidden">
          ...
          <p className="mt-5 text-xs font-bold uppercase tracking-wide text-faint">Plan Your Ride</p>
          <div className="mt-2 flex flex-col">
    ...
    <footer className="bg-ink text-white/70">
      <div className="mx-auto max-w-[1440px] px-6 py-16 sm:px-14">
        <div className="rounded-panel border border-white/15 bg-white/5 px-5 py-4 text-center text-[13px] font-semibold text-white/85 sm:text-left">
          Independent guide &mdash; not affiliated with any single golf-cart operator, tour company, or vehicle manufacturer.
        </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-x-8 gap-y-10 sm:grid-cols-3 lg:grid-cols-5">
          <div className="col-span-2 sm:col-span-3 lg:col-span-1">
            <Link href="/" className="flex items-center gap-2.5">
              <GCRBrandMark />
              <span className="font-sans text-lg font-extrabold tracking-tight text-white">Golf Cart Rome</span>
            </Link>
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-white/55">
              Independent, first-hand guide to seated, accessible golf-cart tours in Rome &mdash; part of the Italy
              Tours network of Rome and Italy travel guides.
            </p>
          ...
          <div>
            <h3 className="text-xs font-bold uppercase tracking-[0.14em] text-white/40">Plan Your Ride</h3>
            <ul className="mt-4 space-y-3">
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-white/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-white/40">&copy; {new Date().getFullYear()} Golf Cart Rome. All rights reserved.</p>
 ...
 * other bespoke property. Whether a listing runs a wheelchair-accessible
 * cart (no-transfer boarding) is this property's equivalent of Rome Vespa's
 * sidecar flag — the single biggest factor in who a listing actually suits.
 */
      ...
      <table className="w-full min-w-[640px] border-collapse text-left text-sm">
        ...
        <thead>
          <tr className="border-b border-line bg-paper-tint">
            ...
            <th scope="col" className="px-4 py-3 font-bold text-ink">Wheelchair boarding</th>
            ...
            <tr key={tour.slug} className="border-b border-line last:border-b-0 even:bg-paper-tint/50">
              ...
              <td className="px-4 py-3">
                {tour.wheelchairAccessible ? (
                  ...
                  <span className="text-faint">Requires seat transfer</span>
                )}
              </td>
...
/**
 * "Before It Was a Tour" stat strip — verifiable historical/mechanical facts
 * about the electric golf cart itself (see QUICK_FACTS in
 * lib/golf-cart-rome.ts), not business metrics.
 */
 ...
 * AtAGlanceItem in lib/golf-cart-rome-content.ts).
 */
...
/** Photo-led gallery grid — reused on the About page (see GCRAboutPage). */

...
import { AtAGlanceBox, TourComparisonTable, GCRAuthorBox, GCRFooter, GCRHeader } from './GCRShared';
import { getFeaturedToursForPage } from '@/lib/golf-cart-rome';
import type { MoneyPageContent } from '@/lib/golf-cart-rome-content';

/**
 * Shared template for all 5 Golf Cart Rome money pages — mirrors Rome
 * Vespa's/Private Vatican's MoneyPageTemplate.tsx exactly in structure
 * (TouristTrip + Product/Offer + FAQPage + BreadcrumbList schema, tour
 ...
 * (blueprint §3.7.2). The `.gcr-scope` wrapper (see globals.css) scopes the
 * Sunny Amber accent tokens to this subtree only.
 */
export function MoneyPageTemplate({ content }: { content: MoneyPageContent }) {
  ...
  // order — same pattern as Rome Vespa's getFeaturedToursForPage.
  ...
  const breadcrumbJsonLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://golfcartrome.com/' },
      { '@type': 'ListItem', position: 2, name: content.navTitle, item: `https://golfcartrome.com${content.href}` },
    ],
  };
  ...
  const touristTripJsonLd = {
    ...
    url: `https://golfcartrome.com${content.href}`,
  };
  ...
  const productJsonLd = relevantTours.map((tour) => ({
    ...
    offers: {
      ...
      url: `https://golfcartrome.com/go/${tour.slug}`,
      availability: 'https://schema.org/InStock',
    },
  }));

  return (
    <div className="gcr-scope bg-white pb-20 lg:pb-0">
      ...
      <GCRHeader toursHref="/#tours" planHref="/#ride-prep" faqHref="#faq" ctaHref="#tour-options" />
      ...
      <section id="tour-options" className="scroll-mt-[65px] border-b border-line bg-paper-tint py-14 sm:py-16">
        <div className="mx-auto max-w-[1200px] px-6 sm:px-14">
          ...
          <p className="mt-3 max-w-[62ch] text-base leading-relaxed text-ink-muted">
            The listings most relevant to this page, side by side — including whether the cart requires a wheelchair
            transfer or supports no-transfer boarding.
          </p>
          <div className="mt-6">

...
import { FEATURED_TOURS } from '@/lib/tuscany-day-trip';

export function TDTToursPage() {
  const jsonLd = {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    name: 'Tuscany Day Trips',
    description: 'Featured Tuscany day trips from Florence — wine tours, hill towns, and countryside routes',
    url: 'https://tuscanydaytrip.com/tours',
    mainEntity: {
      '@type': 'ItemList',
      ...
      <section className="border-b border-line">
        <div className="mx-auto max-w-[1440px] px-6 pt-6 sm:px-14">
          <nav aria-label="Breadcrumb" className="text-sm text-faint">
            ...
            <span className="text-ink-muted">All Day Trips</span>
          </nav>
        </div>
        <div className="mx-auto max-w-[1440px] px-6 py-14 text-center sm:px-14 sm:py-16">
          <h1 className="font-display text-3xl font-semibold text-ink sm:text-4xl">Featured Tuscany Tours</h1>
          <p className="mt-4 text-ink-muted">Curated Tuscany day trips from Florence — wine, hill towns, and countryside routes</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1440px] px-6 sm:px-14">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {FEATURED_TOURS.map((tour) => {
              let moneyPageHref = '/tuscany-day-trip/money/tuscany-from-florence';
              if (tour.tags.includes('wine')) moneyPageHref = '/tuscany-day-trip/money/tuscany-wine-day-trip';
              if (tour.tags.includes('siena-sg')) moneyPageHref = '/tuscany-day-trip/siena-san-gimignano-chianti';

 ...
 */
const SIBLING_SITES = NETWORK_SITES.filter((site) => site.slug !== 'tuscany-day-trip');
...
/**
 * Shared chrome for every Tuscany Day Trip page — home and all 11 money /
 * support / utility pages built off this property's build brief. Mirrors
 * TCShared.tsx's structure exactly, themed via the `.tdt-scope` wrapper each
 ...
 * bespoke Chianti-wine palette.
 */

export function TDTBrandMark() {
  return (
 ...
 */
export function TDTHeader({ toursHref = '#tours', planHref = '#plan-your-trip', faqHref = '#faq', ctaHref = '#featured-tours' }: {
  toursHref?: string;
  planHref?: string;
  faqHref?: string;
  ctaHref?: string;
    ...
    <footer className="bg-ink text-white/70">
      <div className="mx-auto max-w-[1440px] px-6 py-16 sm:px-14">
        <div className="rounded-panel border border-white/15 bg-white/5 px-5 py-4 text-center text-[13px] font-semibold text-white/85 sm:text-left">
          Independent guide &mdash; not affiliated with any winery, tour operator, GetYourGuide, Viator, or Civitatis.
        </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-x-8 gap-y-10 sm:grid-cols-3 lg:grid-cols-5">
          <div className="col-span-2 sm:col-span-3 lg:col-span-1">
            <Link href="/" className="flex items-center gap-2.5">
              <TDTBrandMark />
              <span className="font-display text-lg font-bold tracking-tight text-white">Tuscany Day Trip</span>
            </Link>
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-white/55">
              Independent, first-hand guide to Tuscany day trips from Florence &mdash; part of the Italy Tours network
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-white/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-white/40">&copy; {new Date().getFullYear()} Tuscany Day Trip. All rights reserved.</p>
 ...
 * wine-tasting stop is this property's equivalent of Tiramisù Class's
      ...
      <table className="w-full min-w-[640px] border-collapse text-left text-sm">
        ...
        <thead>
          <tr className="border-b border-line bg-paper-tint">
            ...
            <th scope="col" className="px-4 py-3 font-bold text-ink">Wine Tasting</th>
 ...
 * about Tuscany and Chianti itself (see QUICK_FACTS in
 * lib/tuscany-day-trip.ts), not business metrics.
 */
 ...
 * AtAGlanceItem in lib/tuscany-day-trip-content.ts).
 */
...
/** Photo-led gallery grid — reused on the About page (see TDTAboutPage). */

 ...
 * matching Underground Colosseum/Private Vatican/Pompeii Day Trip's
 * SIBLING_SITES/NETWORK_NAV_ITEMS pattern exactly.
 */
const SIBLING_SITES = NETWORK_SITES.filter((site) => site.slug !== 'rome-vespa');
...
/**
 * Shared chrome for every Rome Vespa page — home and all 12 money / support /
 * utility pages built off the property's blueprint (Master Network Blueprint
 * §3.3). Mirrors PVShared.tsx / PDTShared.tsx's structure, themed with
 * Riviera Teal (`--accent`, scoped via the `.rv-scope` wrapper each top-level
 * page component applies — see globals.css) and Public Sans extrabold
 * headings (blueprint §3.3.2 — the sans-serif, kinetic register, not this
 * network's Fraunces-serif day-trip/monument properties).
 *
 * Trademark note (blueprint §3.3.1): the brand mark/wordmark below reads
 * "Rome Scooter Tours" rather than "Rome Vespa" — the network registry's
 * internal name for this property (lib/tours.ts) and this site's own SEO
 * <title> tags still say "Vespa" per the blueprint's verbatim copy, but the
 * site's own visual brand mark avoids foregrounding Piaggio's trademarked
 * product name as a designed logotype. Every other on-page use of "Vespa" is
 * plain descriptive body copy (the vehicle a tour rides), never a stylized
 * brand element.
 */

export function RVBrandMark() {
  return (
    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-accent-gradient shadow-card-soft">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <circle cx="7" cy="17" r="2.6" stroke="white" strokeWidth="1.6" />
        <circle cx="17" cy="17" r="2.6" stroke="white" strokeWidth="1.6" />
        <path
          d="M9.3 17h5.4M14.7 17 13 9.5h-2.6M13 9.5h3.4c1.6 0 2.9 1.1 3.2 2.7l.4 2M7 14.8V12c0-1 .8-1.8 1.8-1.8H11"
          stroke="white"
          strokeWidth="1.6"
          strokeLinecap="round"
 ...
 */
export function RVHeader({ toursHref = '#tours', planHref = '#ride-prep', faqHref = '#faq', ctaHref = '#featured-tours' }: {
  toursHref?: string;
  planHref?: string;
  faqHref?: string;
  ctaHref?: string;
    ...
    >
      <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between gap-4 px-6 sm:px-14">
        ...
        <nav aria-label="Primary" className="hidden min-w-0 items-center gap-8 lg:flex">
          ...
          <NavDropdown label="Ride Prep" items={PLAN_NAV_ITEMS} footerHref={planHref} footerLabel="See all guides" />
      ...
      {mobileOpen ? (
        <div className="max-h-[calc(100vh-72px)] overflow-y-auto border-t border-line bg-white px-6 py-5 sm:px-14 lg:hidden">
          ...
          <p className="mt-5 text-xs font-bold uppercase tracking-wide text-faint">Ride Prep</p>
          <div className="mt-2 flex flex-col">
    ...
    <footer className="bg-ink text-white/70">
      <div className="mx-auto max-w-[1440px] px-6 py-16 sm:px-14">
        <div className="rounded-panel border border-white/15 bg-white/5 px-5 py-4 text-center text-[13px] font-semibold text-white/85 sm:text-left">
          Independent guide &mdash; not affiliated with Piaggio, any Vespa dealer, or any tour operator we compare.
        </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-x-8 gap-y-10 sm:grid-cols-3 lg:grid-cols-5">
          <div className="col-span-2 sm:col-span-3 lg:col-span-1">
            <Link href="/" className="flex items-center gap-2.5">
              <RVBrandMark />
              <span className="font-sans text-lg font-extrabold tracking-tight text-white">Rome Scooter Tours</span>
            </Link>
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-white/55">
              Independent, first-hand guide to Vespa and sidecar tours in Rome &mdash; part of the Italy Tours network
              of Rome and Italy travel guides.
            </p>
            <p className="mt-4 text-xs font-semibold uppercase tracking-[0.1em] text-white/35">
              GetYourGuide &middot; Viator
            </p>
          </div>
          ...
          <div>
            <h3 className="text-xs font-bold uppercase tracking-[0.14em] text-white/40">Ride Prep</h3>
            <ul className="mt-4 space-y-3">
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-white/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-white/40">&copy; {new Date().getFullYear()} Rome Scooter Tours. All rights reserved.</p>
 ...
 * spec: "tour-comparison-table"), matching Underground Colosseum / Private
 * Vatican's shipped pattern. Whether a listing is a sidecar (chauffeured, no
 * licence needed) is this property's equivalent of PV's early-access flag —
 * the single biggest factor in who a Vespa tour listing actually suits.
 */
      ...
      <table className="w-full min-w-[640px] border-collapse text-left text-sm">
        ...
        <thead>
          <tr className="border-b border-line bg-paper-tint">
            ...
            <th scope="col" className="px-4 py-3 font-bold text-ink">Sidecar</th>
            ...
            <tr key={tour.slug} className="border-b border-line last:border-b-0 even:bg-paper-tint/50">
              ...
              <td className="px-4 py-3">
                {tour.sidecar ? (
                  ...
                  <span className="text-faint">Rider licence required</span>
                )}
              </td>
...
/**
 * "Before It Was a Tour" stat strip — verifiable historical facts about the
 * Vespa scooter itself (see QUICK_FACTS in lib/rome-vespa.ts), not business
 * metrics.
 */
 ...
 * AtAGlanceItem in lib/rome-vespa-content.ts).
 */
...
/** Photo-led gallery grid — reused on the About page (see RVAboutPage). */

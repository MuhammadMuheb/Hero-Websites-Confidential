...
import { AtAGlanceBox, TourComparisonTable, CIRAuthorBox, CIRFooter, CIRHeader } from './CIRShared';
import { getFeaturedToursForPage } from '@/lib/cooking-in-rome';
import type { MoneyPageContent } from '@/lib/cooking-in-rome-content';

/**
 * Shared template for all 5 Cooking in Rome money pages — mirrors Golf Cart
 * Rome's/Rome Vespa's MoneyPageTemplate.tsx exactly in structure
 * (BreadcrumbList + FAQPage + Course/Event + Product/Offer schema, per this
 * property's stated schema types — blueprint §3.8.1 — not TouristTrip), tour
 ...
 * (blueprint §3.8.2). The `.cir-scope` wrapper (see globals.css) scopes the
 * Basil Green accent tokens to this subtree only.
 */
export function MoneyPageTemplate({ content }: { content: MoneyPageContent }) {
  ...
  // order — same pattern as Golf Cart Rome's getFeaturedToursForPage.
  ...
  const breadcrumbJsonLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://cookinginrome.com/' },
      { '@type': 'ListItem', position: 2, name: content.navTitle, item: `https://cookinginrome.com${content.href}` },
    ],
  };
  ...
  const courseJsonLd = {
    ...
    hasCourseInstance: relevantTours.map((tour) => ({
      '@type': 'CourseInstance',
      courseMode: 'onsite',
      name: tour.title,
      url: `https://cookinginrome.com/go/${tour.slug}`,
    })),
  };
  ...
  return (
    <div className="cir-scope bg-white pb-20 lg:pb-0">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbJsonLd) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(courseJsonLd) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(productJsonLd) }} />

      <CIRHeader toursHref="/#classes" planHref="/#before-you-book" faqHref="#faq" ctaHref="#class-options" />
      ...
      <section id="class-options" className="scroll-mt-[65px] border-b border-line bg-paper-tint py-14 sm:py-16">
        <div className="mx-auto max-w-[1200px] px-6 sm:px-14">
          ...
          <p className="mt-3 max-w-[62ch] text-base leading-relaxed text-ink-muted">
            The listings most relevant to this page, side by side — including whether the class builds in a real
            market visit before cooking.
          </p>
          <div className="mt-6">

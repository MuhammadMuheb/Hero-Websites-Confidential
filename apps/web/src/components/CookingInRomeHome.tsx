...
/**
 * Homepage for the Cooking in Rome hero property (cookinginrome.com, served
...
function MoneyPagesCarousel() {
  ...
  return (
    <section id="classes" className="scroll-mt-[65px] border-b border-line py-16 sm:py-20">
      <div className="mx-auto max-w-[1200px] px-6 sm:px-14">
        <div className="flex flex-wrap items-end justify-between gap-y-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-accent">The 5 ways to learn</p>
            <h2 className="mt-2 font-sans text-[28px] font-extrabold leading-[1.15] tracking-tight text-ink sm:text-[36px]">
              Choose Your Rome Cooking Class
            </h2>
          </div>
          <CarouselArrows label="class page" onPrev={() => scrollByOneCard(-1)} onNext={() => scrollByOneCard(1)} />
        </div>
...
function FeaturedToursCarousel() {
  ...
  return (
    <section id="featured-classes" className="scroll-mt-[65px] border-b border-line bg-paper-tint py-16 sm:py-20">
      <div className="mx-auto max-w-[1200px] px-6 sm:px-14">
        <div className="flex flex-wrap items-end justify-between gap-y-4">
          <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <CarouselArrows label="class" onPrev={() => scrollByOneCard(-1)} onNext={() => scrollByOneCard(1)} />
          </div>
        </div>
        <p className="mt-3 max-w-[62ch] text-[15.5px] leading-relaxed text-ink-muted">
          Every card pulls partner, price band, and duration live from the class listings we track.
        </p>
        <p className="mt-2 flex items-center gap-1.5 text-sm font-semibold text-gold">
          ...
          Market-visit and evening slots book out first — reserve at least a week ahead in peak season.
        </p>
...
export function CookingInRomeHome() {
  const websiteJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'WebSite',
    name: 'Cooking in Rome',
    url: 'https://cookinginrome.com/',
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: MONEY_PAGES.map((page, index) => ({
      ...
      position: index + 1,
      name: page.title,
      url: `https://cookinginrome.com${page.href}`,
    })),
  };
  ...
  const courseJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Course',
    name: 'Rome Cooking Classes',
    description:
      "Independent guide to Rome cooking classes — pasta, pizza + gelato, market-to-table, and private/small-group options, every class taken in person before recommending it.",
    provider: { '@type': 'Organization', name: 'Cooking in Rome', sameAs: 'https://cookinginrome.com/' },
    url: 'https://cookinginrome.com/',
  };
  ...
  return (
    <div className="cir-scope bg-white">
        ...
        <div className="mx-auto grid max-w-[1200px] gap-10 px-6 py-14 sm:px-14 lg:grid-cols-[1.05fr_1fr] lg:items-center lg:py-20">
          <div>
            ...
            <h1 className="mt-4 font-sans text-[38px] font-extrabold leading-[1.08] tracking-tight text-ink sm:text-[54px]">
              Rome Cooking Classes, Compared by Someone Who&rsquo;s Taken Them
            </h1>
            <p className="mt-5 max-w-[52ch] text-[17px] leading-relaxed text-ink-muted">
              Pasta, pizza, market-to-table &mdash; every class on this site has been taken in person, so you know
              exactly what you&rsquo;re walking into before you book.
            </p>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              >
                What does a class actually include?
              </Link>
            </div>
            ...
            <div className="mt-8 flex flex-col gap-2.5 text-sm text-ink-muted sm:flex-row sm:flex-wrap sm:gap-x-6 sm:gap-y-2">
              {[
                ...
                'Vegetarian options clearly marked, not buried',
                'Market-visit vs studio-only classes distinguished',
                'Gift-experience booking made simple',
              ].map((label) => (
      ...
      <section className="border-b border-line bg-paper-tint py-14 sm:py-16">
        <div className="mx-auto max-w-[1200px] px-6 sm:px-14">
          ...
          <h2 className="mt-2 font-sans text-[26px] font-extrabold leading-snug tracking-tight text-ink sm:text-[32px]">
            Roman Cuisine, by the Numbers
          </h2>
          <p className="mt-3 max-w-[62ch] text-[15.5px] leading-relaxed text-ink-muted">
            A little history before you book — the cuisine these classes actually teach.
          </p>
          <div className="mt-8">
      ...
      {/* ---------- full class comparison table ---------- */}
      <section className="border-b border-line py-16 sm:py-20">
        <div className="mx-auto max-w-[1200px] px-6 sm:px-14">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-accent">Every partner, side by side</p>
          <h2 className="mt-2 font-sans text-[28px] font-extrabold leading-[1.15] tracking-tight text-ink sm:text-[36px]">
            Compare All 8 Classes at a Glance
          </h2>
          <p className="mt-3 max-w-[62ch] text-[15.5px] leading-relaxed text-ink-muted">
            The same 8 listings as the carousel above, laid out as a straight spec comparison — duration, whether
            the class includes a real market visit, and price.
          </p>
          <div className="mt-8">
            <TourComparisonTable tours={FEATURED_TOURS} caption="Comparison of every featured Cooking in Rome class by partner, duration, market visit and price" />
          </div>
        </div>
      </section>

      {/* ---------- before you book / support pages ---------- */}
      <section id="before-you-book" className="scroll-mt-[65px] border-b border-line bg-paper-tint py-16 sm:py-20">
        <div className="mx-auto max-w-[1200px] px-6 sm:px-14">

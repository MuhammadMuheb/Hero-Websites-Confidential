    ...
    <div className="nsf-scope bg-white">
      ...
      <section className="border-b border-line">
        <div className="mx-auto grid max-w-[1200px] gap-10 px-6 py-14 sm:px-14 lg:grid-cols-[1.05fr_1fr] lg:items-center lg:py-20">
          <div>
            {HERO.eyebrow && <p className="text-xs font-semibold uppercase tracking-[0.14em] text-faint">{HERO.eyebrow}</p>}
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link href={HERO.primaryCta.href} className="flex h-11 items-center justify-center rounded-control bg-accent-gradient px-6 text-sm font-bold text-white shadow-glow transition-transform duration-200 ease-out hover:scale-[1.02] active:scale-[0.98]">
                {HERO.primaryCta.label}
              </Link>
              <Link href={HERO.secondaryCta.href} className="flex h-11 items-center justify-center rounded-control border border-accent px-6 text-sm font-medium text-accent transition-colors hover:bg-accent-soft">
          ...
          <div className="relative aspect-[4/3] overflow-hidden rounded-media bg-media shadow-card">
            <SafeImage src={HERO_IMAGE.src} alt={HERO_IMAGE.alt} fill priority sizes="(min-width: 1024px) 560px, 100vw" className="object-cover" />
          </div>
        </div>
      </section>
      ...
      <div className="border-b border-line bg-white">
        <div className="mx-auto flex max-w-[1200px] items-center gap-2 overflow-x-auto px-6 py-4 [scrollbar-width:none] sm:px-14 [&::-webkit-scrollbar]:hidden">
          {QUICK_LINKS.map((link) => (
            <Link key={link.href} href={link.href} className="flex h-9 shrink-0 items-center whitespace-nowrap rounded-full border border-line px-3.5 text-sm font-semibold text-ink-muted transition-all duration-200 ease-out hover:border-accent hover:bg-accent-soft hover:text-accent">
              {link.label}
            </Link>
          ))}
        </div>

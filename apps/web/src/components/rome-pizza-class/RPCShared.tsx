 ...
 * Rome/Cooking in Rome's SIBLING_SITES/NETWORK_NAV_ITEMS pattern exactly.
 */
const SIBLING_SITES = NETWORK_SITES.filter((site) => site.slug !== 'rome-pizza-class');
...
/**
 * Shared chrome for every Rome Pizza Class page — home and all 10 money /
 * support / utility pages built off the property's blueprint (Master Network
 * Blueprint §3.9). Mirrors GCRShared.tsx/CIRShared.tsx's structure, themed
 * with Tomato Red (`--accent`, scoped via the `.rpc-scope` wrapper each
 * top-level page component applies — see globals.css) and Public Sans
 * extrabold headings in a playful weight (blueprint §3.9.2) — the pizza-only
 * single-dish framing that deliberately narrows against Cooking in Rome's
 * broad-hub positioning.
 */

export function RPCBrandMark() {
  return (
 ...
 */
export function RPCHeader({ toursHref = '#tours', planHref = '#pizza-prep', faqHref = '#faq', ctaHref = '#featured-tours' }: {
  toursHref?: string;
  planHref?: string;
  faqHref?: string;
  ctaHref?: string;
    ...
    <footer className="bg-ink text-white/70">
      <div className="mx-auto max-w-[1440px] px-6 py-16 sm:px-14">
        <div className="rounded-panel border border-white/15 bg-white/5 px-5 py-4 text-center text-[13px] font-semibold text-white/85 sm:text-left">
          Independent guide &mdash; not affiliated with any single pizzeria, cooking school, GetYourGuide, or Viator.
        </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-x-8 gap-y-10 sm:grid-cols-3 lg:grid-cols-5">
          <div className="col-span-2 sm:col-span-3 lg:col-span-1">
            <Link href="/" className="flex items-center gap-2.5">
              <RPCBrandMark />
              <span className="font-sans text-lg font-extrabold tracking-tight text-white">Rome Pizza Class</span>
            </Link>
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-white/55">
              Independent, first-hand guide to hands-on pizza-making classes in Rome &mdash; part of the Italy Tours
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-white/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-white/40">&copy; {new Date().getFullYear()} Rome Pizza Class. All rights reserved.</p>
 ...
 * other bespoke property. Whether a listing bakes in a genuine wood-fired
 * oven is this property's equivalent of Golf Cart Rome's wheelchairAccessible
 * flag and Cooking in Rome's marketVisit flag — the single biggest factor
 * pizza-focused visitors ask about first.
 */
      ...
      <table className="w-full min-w-[640px] border-collapse text-left text-sm">
        ...
        <thead>
          <tr className="border-b border-line bg-paper-tint">
            ...
            <th scope="col" className="px-4 py-3 font-bold text-ink">Oven type</th>
            ...
            <tr key={tour.slug} className="border-b border-line last:border-b-0 even:bg-paper-tint/50">
              ...
              <td className="px-4 py-3">
                {tour.woodFiredOven ? (
                  ...
                  <span className="text-faint">Electric oven</span>
                )}
              </td>
...
/**
 * "Before You Book" stat strip — verifiable historical/technique facts about
 * pizza itself (see QUICK_FACTS in lib/rome-pizza-class.ts), not business
 * metrics.
 */
 ...
 * AtAGlanceItem in lib/rome-pizza-class-content.ts).
 */
...
/** Photo-led gallery grid — reused on the About page (see RPCAboutPage). */

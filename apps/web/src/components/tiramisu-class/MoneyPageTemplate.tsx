...
/**
 * Shared template for all 4 Tiramisù Class money pages — mirrors Rome Pizza
 * Class's MoneyPageTemplate.tsx exactly in structure (Course/Event +
 * Product/Offer + FAQPage + BreadcrumbList schema, class comparison cards, a
 * verdict block, FAQ, one contextual link), themed with Fraunces display
 * serif headings at smaller sizes per this property's design tokens
 * (blueprint §3.10.2). The `.tc-scope` wrapper (see globals.css) scopes the
 * Cocoa Brown accent tokens and the warm cream paper-tint shift to this
 * subtree only.
 */
export function MoneyPageTemplate({ content }: { content: MoneyPageContent }) {
  ...
  // order — same pattern as Rome Pizza Class's getFeaturedToursForPage.
  ...
  // Course/Event schema per this property's blueprint (§3.10.1) — a
  ...
  const courseJsonLd = {
    ...
    url: `https://tiramisuclass.com${content.href}`,
    hasCourseInstance: {
      '@type': 'CourseInstance',
      courseMode: 'Onsite',
  ...
  const productJsonLd = relevantTours.map((tour) => ({
    ...
    offers: {
      ...
      url: `https://tiramisuclass.com/go/${tour.slug}`,
      availability: 'https://schema.org/InStock',
    },
  }));

  return (
    <div className="tc-scope bg-white pb-20 lg:pb-0">
      ...
      <section id="class-options" className="scroll-mt-[65px] border-b border-line bg-paper-tint py-14 sm:py-16">
        <div className="mx-auto max-w-[1200px] px-6 sm:px-14">
          ...
          <p className="mt-3 max-w-[62ch] text-base leading-relaxed text-ink-muted">
            The listings most relevant to this page, side by side — including whether you taste a chilled sample in
            class or take your tiramisù home to fully set.
          </p>
          <div className="mt-6">

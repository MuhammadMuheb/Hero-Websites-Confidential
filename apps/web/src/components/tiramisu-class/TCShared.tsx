 ...
 * Colosseum/Private Vatican/Pompeii Day Trip/Rome Vespa/Golf Cart
 * Rome/Cooking in Rome/Rome Pizza Class's SIBLING_SITES/NETWORK_NAV_ITEMS
 * pattern exactly.
 */
const SIBLING_SITES = NETWORK_SITES.filter((site) => site.slug !== 'tiramisu-class');
...
/**
 * Shared chrome for every Tiramisù Class page — home and all 10 money /
 * support / utility pages built off the property's blueprint (Master Network
 * Blueprint §3.10). Mirrors RPCShared.tsx's structure, themed with Cocoa
 * Brown (`--accent`, scoped via the `.tc-scope` wrapper each top-level page
 * component applies — see globals.css) and Fraunces display serif at
 * smaller sizes (blueprint §3.10.2) — a touch more delicate/patisserie-coded
 * than Rome Pizza Class's bold sans, matching this property's narrower,
 * single-dessert framing.
 */

export function TCBrandMark() {
  return (
    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-accent-gradient shadow-card-soft">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <rect x="6" y="5" width="12" height="14" rx="1.5" stroke="white" strokeWidth="1.6" />
        <path d="M6 10.5h12" stroke="white" strokeWidth="1.4" />
        <path d="M6 14.5h12" stroke="white" strokeWidth="1.4" />
 ...
 */
export function TCHeader({ toursHref = '#tours', planHref = '#dessert-prep', faqHref = '#faq', ctaHref = '#featured-tours' }: {
  toursHref?: string;
  planHref?: string;
  faqHref?: string;
  ctaHref?: string;
    ...
    >
      <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between gap-4 px-6 sm:px-14">
        ...
        <nav aria-label="Primary" className="hidden min-w-0 items-center gap-8 lg:flex">
          <NavDropdown label="Classes" items={TOURS_NAV_ITEMS} footerHref={toursHref} footerLabel="Compare all 4 classes" />
    ...
    <footer className="bg-ink text-white/70">
      <div className="mx-auto max-w-[1440px] px-6 py-16 sm:px-14">
        <div className="rounded-panel border border-white/15 bg-white/5 px-5 py-4 text-center text-[13px] font-semibold text-white/85 sm:text-left">
          Independent guide &mdash; not affiliated with any single pastry school, restaurant, GetYourGuide, or Viator.
        </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-x-8 gap-y-10 sm:grid-cols-3 lg:grid-cols-5">
          <div className="col-span-2 sm:col-span-3 lg:col-span-1">
            <Link href="/" className="flex items-center gap-2.5">
              <TCBrandMark />
              <span className="font-display text-lg font-bold tracking-tight text-white">Tiramisù Class</span>
            </Link>
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-white/55">
              Independent, first-hand guide to hands-on tiramisù and dessert classes in Rome &mdash; part of the
              Italy Tours network of Rome and Italy travel guides.
            </p>
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-white/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-white/40">&copy; {new Date().getFullYear()} Tiramisù Class. All rights reserved.</p>
 ...
 * other bespoke property. Whether a listing lets you taste a chilled sample
 * in class (rather than sending you home with a still-setting tiramisù) is
 * this property's equivalent of Rome Pizza Class's woodFiredOven flag,
 * Golf Cart Rome's wheelchairAccessible flag, and Cooking in Rome's
 * marketVisit flag — the single biggest factor visitors ask about first,
 * since tiramisù is a no-bake, no-cook dessert with no oven or stove
 * comparison to make.
 */
      ...
      <table className="w-full min-w-[640px] border-collapse text-left text-sm">
        ...
        <thead>
          <tr className="border-b border-line bg-paper-tint">
            ...
            <th scope="col" className="px-4 py-3 font-bold text-ink">Tasting</th>
              ...
              <td className="px-4 py-3">
                {tour.eatInClass ? (
                  <span className="inline-flex items-center gap-1 font-semibold text-success">
                    ...
                    Chilled sample in class
                  </span>
                ) : (
                  <span className="text-faint">Takes home to set</span>
                )}
              </td>
...
/**
 * "Before You Book" stat strip — verifiable historical/ingredient facts
 * about tiramisù itself (see QUICK_FACTS in lib/tiramisu-class.ts), not
 * business metrics.
 */
 ...
 * AtAGlanceItem in lib/tiramisu-class-content.ts).
 */
...
/** Photo-led gallery grid — reused on the About page (see TCAboutPage). */

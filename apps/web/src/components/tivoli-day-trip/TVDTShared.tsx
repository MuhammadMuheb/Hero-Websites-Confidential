 ...
 */
const SIBLING_SITES = NETWORK_SITES.filter((site) => site.slug !== 'tivoli-day-trip');
...
/**
 * Shared chrome for every Tivoli Day Trip page — home and all 11 money /
 ...
 */
export function TVDTHeader({ toursHref = '#tours', planHref = '#plan-your-trip', faqHref = '#faq', ctaHref = '#featured-tours' }: {
  toursHref?: string;
  planHref?: string;
  faqHref?: string;
  ctaHref?: string;
    ...
    <footer className="bg-ink text-white/70">
      <div className="mx-auto max-w-[1440px] px-6 py-16 sm:px-14">
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-white/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-white/40">&copy; {new Date().getFullYear()} Tivoli Day Trip. All rights reserved.</p>
 ...
 * AtAGlanceItem in lib/tivoli-day-trip-content.ts).
 */
...
/** Photo-led gallery grid — reused on the About page (see TVDTAboutPage). */

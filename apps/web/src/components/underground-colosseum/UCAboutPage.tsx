 ...
 * and money pages.
 *
 * Revamped from a single plain text block into a full trust/conversion
 * section: this platform's pages exist to move a reader toward a specific
 * booking decision, not just to explain who wrote the site, so About needs
 * the same "here's exactly where to go next" structure as the homepage and
 * money pages — a stat grid backing up the independence claim with real
 * numbers (derived from MONEY_PAGES/SUPPORT_PAGES, not invented), a photo
 * grid reusing the same first-hand Colosseum photography as the Arena Floor
 * Walkthrough page (see PhotoGallery in UCShared) so the "walked every
 * circuit in person" claim isn't just asserted in text, and a card grid
 * that sends the reader straight into the comparison pages instead of
 * dead-ending on a bio.
 */

const GUIDE_COUNT = SUPPORT_PAGES.length + 2; // + Arena Floor Walkthrough + Is It Worth It (both dedicated pages, not in SUPPORT_PAGES)

const TRUST_STATS = [
  { value: String(MONEY_PAGES.length), label: 'Tour comparison guides', detail: 'One per booking decision — best format, best price, best fit.' },
  { value: String(GUIDE_COUNT), label: 'Planning guides', detail: 'Opening hours, access rules, routes, and the honest verdict.' },
...
const CREDENTIALS = [
  'Walked every underground and arena-floor route this site compares, in person',
  'Booked and paid for tickets the same way a reader would — no press comps',
  'Cross-checks each operator’s current inclusions before publishing a verdict',
        ...
        <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-center gap-10 px-6 py-10 sm:px-14 sm:py-14 lg:grid-cols-2 lg:gap-14">
          <div>
            ...
            <h1 className="mt-3 font-sans text-[34px] font-extrabold leading-[1.1] tracking-tight text-ink sm:text-[42px]">
              The Colosseum Underground Guide Written by Someone Who Actually Walked Every Circuit
            </h1>
            <p className="mt-5 text-[17px] leading-relaxed text-ink-muted">
              Most &ldquo;guides&rdquo; to the Colosseum&rsquo;s underground and arena-floor tours are written by
              whoever sells the ticket. This one isn&rsquo;t. Every comparison on this site comes from first-hand
              research — the same routes, cross-checked across every major operator, published without a single
              paid placement.
            </p>
      ...
      <section className="border-b border-line py-14 sm:py-16">
        ...
        <div className="mx-auto mt-10 max-w-[1200px] px-6 sm:px-14">
          <p className="text-xs font-semibold uppercase tracking-wide text-faint">
            Photographed on every visit — arena floor and hypogeum, no stock substitutes
          </p>
          <div className="mt-4">
            ...
            {MONEY_PAGES.map((page) => (
              ...
              >
                ...
                <div className="flex flex-1 flex-col p-5">
                  <h3 className="font-sans text-[16px] font-bold leading-snug text-ink">{page.title}</h3>
          ...
          <div className="flex flex-col items-start gap-6 rounded-2xl border border-line bg-paper-tint p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
            <div>
              ...
              <p className="mt-2 max-w-[52ch] text-sm leading-relaxed text-ink-muted">
                ...
                arena-floor access is actually included.
              </p>
            </div>
            <Link
              href="/#featured-tours"

...
import { PVAuthorBox, PVFooter, PVHeader } from './PVShared';
import type { SupportPageContent } from '@/lib/private-vatican-content';

/**
 * Shared template for all 5 Private Vatican support pages — mirrors
 * Underground Colosseum's SupportPageTemplate.tsx exactly in structure
 * (FAQPage + BreadcrumbList schema, first-hand detail, one primary CTA into
 * the money page it feeds, one contextual link, FAQ). Content is entirely
 * data-driven (lib/private-vatican-content.ts).
 */
export function SupportPageTemplate({ content }: { content: SupportPageContent }) {
  ...
  return (
    <div className="pv-scope bg-white">
      ...
      <PVHeader toursHref="/#tours" planHref="/#plan-your-visit" faqHref="#faq" ctaHref={`${content.relatedMoneyHref}#tour-options`} />
          ...
          <div className="flex flex-col items-start gap-6 rounded-media border border-line bg-paper-tint p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
            <div>
              ...
              <p className="mt-2 max-w-[52ch] text-sm leading-relaxed text-ink-muted">
                This page feeds directly into our comparison of every tour that covers this — see the full

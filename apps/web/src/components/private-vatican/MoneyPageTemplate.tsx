...
/**
 * Shared template for all 5 Private Vatican money pages — mirrors
 * Underground Colosseum's MoneyPageTemplate.tsx exactly in structure
 * (TouristAttraction + Product/Offer + FAQPage + BreadcrumbList schema, tour
 * comparison cards, a verdict block, FAQ, one contextual link), themed with
 * Fraunces display headings per this property's design tokens (blueprint
 * §3.6.2) instead of Underground Colosseum's deliberate sans-serif H1s. The
 * `.pv-scope` wrapper (see globals.css) scopes the Papal Gold accent tokens
 * to this subtree only.
 */
export function MoneyPageTemplate({ content }: { content: MoneyPageContent }) {
  ...
  // order — same pattern as Underground Colosseum's getFeaturedToursForPage.
  ...
  const breadcrumbJsonLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://privatevatican.com/' },
      { '@type': 'ListItem', position: 2, name: content.navTitle, item: `https://privatevatican.com${content.href}` },
    ],
  };
  ...
  const attractionJsonLd = {
    ...
    url: `https://privatevatican.com${content.href}`,
  };
  ...
  const productJsonLd = relevantTours.map((tour) => ({
    ...
    offers: {
      ...
      url: `https://privatevatican.com/go/${tour.slug}`,
      availability: 'https://schema.org/InStock',
    },
  }));

  return (
    <div className="pv-scope bg-white pb-20 lg:pb-0">
      ...
      <section id="tour-options" className="scroll-mt-[65px] border-b border-line bg-paper-tint py-14 sm:py-16">
        <div className="mx-auto max-w-[1200px] px-6 sm:px-14">
          ...
          <p className="mt-3 max-w-[62ch] text-base leading-relaxed text-ink-muted">
            The listings most relevant to this page, side by side — including whether early access before public
            opening is actually part of the ticket.
          </p>
          <div className="mt-6">

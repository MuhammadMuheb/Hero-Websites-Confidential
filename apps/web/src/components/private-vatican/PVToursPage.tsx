...
import { FEATURED_TOURS } from '@/lib/private-vatican';

export function PVToursPage() {
  const jsonLd = {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    name: 'Private Vatican Tours',
    description: 'Featured Vatican early-access, private, and skip-the-line tours and guides',
    url: 'https://privatevatican.com/tours',
    mainEntity: {
      '@type': 'ItemList',
      ...
      <section className="border-b border-line">
        <div className="mx-auto max-w-[1440px] px-6 pt-6 sm:px-14">
          <nav aria-label="Breadcrumb" className="text-sm text-faint">
            ...
            <span className="text-ink-muted">All Tours</span>
          </nav>
        </div>
        <div className="mx-auto max-w-[1440px] px-6 py-14 text-center sm:px-14 sm:py-16">
          <h1 className="font-display text-3xl font-semibold text-ink sm:text-4xl">Featured Vatican Tours</h1>
          <p className="mt-4 text-ink-muted">Curated Vatican early-access, private, and skip-the-line tours</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1440px] px-6 sm:px-14">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {FEATURED_TOURS.map((tour) => {
              let moneyPageHref = '/money/early-entry-vatican-sistine';
              if (tour.tags.includes('private')) moneyPageHref = '/money/private-vatican-guide';
              if (tour.tags.includes('dome')) moneyPageHref = '/money/vatican-st-peter-s-dome';
              if (tour.tags.includes('family')) moneyPageHref = '/money/vatican-with-kids-family';
              if (tour.tags.includes('skip-the-line')) moneyPageHref = '/money/skip-the-line-explained';

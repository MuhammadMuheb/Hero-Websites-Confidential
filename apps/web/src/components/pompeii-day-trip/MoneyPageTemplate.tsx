...
import { AtAGlanceBox, TourComparisonTable, PDTAuthorBox, PDTFooter, PDTHeader } from './PDTShared';
import { getFeaturedToursForPage } from '@/lib/pompeii-day-trip';
import type { MoneyPageContent } from '@/lib/pompeii-day-trip-content';

/**
 * Shared template for all 6 Pompeii Day Trip money pages — mirrors
 * Underground Colosseum's and Private Vatican's MoneyPageTemplate.tsx exactly
 * in structure (TouristTrip + Product/Offer + FAQPage + BreadcrumbList
 * schema, tour comparison cards, a verdict block, FAQ, one contextual link),
 * themed with Fraunces display headings per this property's design tokens
 * (blueprint §3.2.2). The `.pdt-scope` wrapper (see globals.css) scopes the
 * Volcanic Ember accent tokens to this subtree only.
 */
export function MoneyPageTemplate({ content }: { content: MoneyPageContent }) {
  // Each money page leads with the tours most relevant to its own origin
  // city or topic instead of every page repeating the identical cards in the
  // identical order — same pattern as Underground Colosseum's and Private
  // Vatican's getFeaturedToursForPage.
  ...
  const breadcrumbJsonLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://pompeiidaytrip.com/' },
      { '@type': 'ListItem', position: 2, name: content.navTitle, item: `https://pompeiidaytrip.com${content.href}` },
    ],
  };
  ...
  const touristTripJsonLd = {
    ...
    url: `https://pompeiidaytrip.com${content.href}`,
  ...
  const productJsonLd = relevantTours.map((tour) => ({
    ...
    offers: {
      ...
      url: `https://pompeiidaytrip.com/go/${tour.slug}`,
  ...
  return (
    <div className="pdt-scope bg-white pb-20 lg:pb-0">
      ...
      <PDTHeader toursHref="/#tours" planHref="/#plan-your-trip" faqHref="#faq" ctaHref="#tour-options" />
      ...
      <section className="border-b border-line">
        ...
        <div className="mx-auto grid max-w-[1200px] gap-10 px-6 py-8 sm:px-14 lg:grid-cols-[1.05fr_1fr] lg:items-center lg:py-14">
          <div>
            ...
            <h1 className="mt-3 font-display text-[32px] font-bold leading-[1.1] tracking-tight text-ink sm:text-[42px]">
              {content.h1}
            </h1>
      ...
      <section id="tour-options" className="scroll-mt-[65px] border-b border-line bg-paper-tint py-14 sm:py-16">
        <div className="mx-auto max-w-[1200px] px-6 sm:px-14">
          ...
          <p className="mt-3 max-w-[62ch] text-base leading-relaxed text-ink-muted">
            The listings most relevant to this page, side by side — including whether genuine skip-the-line entry is
            actually part of the ticket.
          </p>
          <div className="mt-6">
      ...
      <section id="faq" className="scroll-mt-[65px] border-b border-line py-14 sm:py-16">
        <div className="mx-auto max-w-[760px] px-6 sm:px-14">
          <h2 className="font-display text-[26px] font-bold leading-snug tracking-tight text-ink sm:text-[32px]">
            Frequently Asked
          </h2>
